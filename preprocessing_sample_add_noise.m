...
adsClean = audioDatastore(fullfile(dataFolder));

reduceDataset = true;
if reduceDataset
    adsClean = shuffle(adsClean);
    adsTrain = subset(adsClean,1:64);
    adsTrain2 = subset(adsClean,67:128);
end
...
index = length(adsNoise.Files)/2;
adsNoise2 = subset(adsNoise,index:length(adsNoise.Files));
adsNoise = subset(adsNoise,2:index);
...
% The function HelperGenerateSpeechDenoisingFeatures generates targets and
% predictors at each cell
[targets,predictors] = cellfun(@(x)HelperGenerateSpeechDenoisingFeatures(x,adsNoise2,src),T,"UniformOutput",false);
targets = targets(cellfun(@(x) ~isequal(x, 0), targets));
predictors = predictors(cellfun(@(x) ~isequal(x, 0), predictors));
...
% Train the network
denoiseNetFullyConnected = trainNetwork(trainPredictors,trainTargets,layerGraph(denoiseNetFullyConnected),options);
